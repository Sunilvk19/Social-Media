...
const PostCard = ({ post, onLike, isLiked, authorMood }) => {
  ...
  return (
    <div className={`glass-card rounded-[40px] p-8 relative overflow-hidden group transition-all hover:translate-y-[-4px] ${tintClass}`}>
      
      {/* Large Floating Mood Emoji (Visual Prime) */}
      {authorMood && (
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className={`w-14 h-14 rounded-full bg-linear-to-tr flex items-center justify-center overflow-hidden shadow-lg border-2 duration-300 ${authorMood ? 'p-1 ' + authorMood.color : 'border-white/10 bg-linear-to-tr from-rose-400 to-pink-500'}`}>
      ...
      <div className="space-y-4">
        {isDistress && (
           <div className="text-4xl mb-2 animate-bounce-subtle">{authorMood.emoji}</div>
        )}
        ...
        {isDistress ? (
          <div className="space-y-6">
            <div className="flex items-center gap-2 text-blue-400 text-xs font-black uppercase tracking-widest">
              ...
              {post.authorName} is feeling {authorMood.label.toLowerCase()} — be there for them
            </div>
            ...
            <div className="flex items-center gap-3 py-2">
              <div className="flex -space-x-2">
                {[1,2,3].map(i => (
                  <div key={i} className="w-6 h-6 rounded-full border-2 border-brand-dark bg-white/10 flex items-center justify-center text-[10px]">👤</div>
                ))}
              </div>
              <p className="text-[11px] font-bold text-white/30">
                <span className="text-white/60">Jordan, Sam</span> & 4 others already reached out
              </p>
            </div>
          </div>
        ) : (
      ...
      {showTalkConfirm && (
        <div className="absolute inset-0 z-50 bg-brand-dark/95 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="text-center space-y-6">
            <div className="w-20 h-20 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto text-3xl">💬</div>
            <div className="space-y-2">
              <h4 className="text-xl font-black">Reach out to {post.authorName.split(' ')[0]}?</h4>
              <p className="text-white/50 text-sm leading-relaxed">
                They are feeling {authorMood.label.toLowerCase()} right now.<br/>A simple message can mean the world.
              </p>
            </div>
            <div className="flex flex-col gap-3">
              ...
              <Button onClick={() => setShowTalkConfirm(false)} className="w-full bg-transparent text-white/40 hover:text-white py-2 font-bold text-sm">
                Maybe later
              </Button>
            </div>
          </div>
